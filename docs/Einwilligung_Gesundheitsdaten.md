# Einwilligung in die Verarbeitung von Gesundheitsdaten

nach Art. 9 Abs. 2 lit. a Datenschutz-Grundverordnung (DSGVO)

---

&nbsp;

## Angaben zur betroffenen Person

**Name, Vorname:** _______________________________________________

**Geburtsdatum:** _______________________________________________

**Anschrift:** _______________________________________________

&nbsp;

---

&nbsp;

## Worum geht es?

Die Firma **[FIRMA]** unterstützt Sie im Alltag als Anbieter von
Alltagshilfe nach dem Pflegeversicherungsgesetz (§ 45b SGB XI).

Damit wir gut für Sie sorgen und die Leistungen mit Ihrer Pflegekasse
abrechnen können, müssen wir einige Ihrer persönlichen Daten speichern.

Manche dieser Daten betreffen Ihre **Gesundheit** — zum Beispiel Ihren
**Pflegegrad** oder Ihre **Blutdruckwerte**. Solche Daten sind besonders
geschützt. Deshalb brauchen wir Ihre ausdrückliche Zustimmung.

&nbsp;

## Welche Daten speichern wir?

Wir speichern folgende Angaben über Sie:

- Ihren **Namen** und Ihre **Anschrift**
- Ihr **Geburtsdatum**
- Ihren **Pflegegrad**
- Ihre **Versichertennummer** und den Namen Ihrer **Pflegekasse**
- Ihre **Bankverbindung** (IBAN) — falls Sie privat zuzahlen
- Welche **Leistungen** wir für Sie erbracht haben (Datum, Dauer, Art)
- Die **Rechnungen** dazu
- Ihre **Blutdruckwerte** (oberer und unterer Wert, Puls, Datum und
  Uhrzeit der Messung) — nur, wenn wir den Blutdruck für Sie messen
  und dokumentieren

&nbsp;

## Warum speichern wir das?

- Damit wir unsere Arbeit für Sie planen und durchführen können
- Damit wir die Leistungen bei Ihrer Pflegekasse abrechnen können
- Damit wir Ihnen Rechnungen erstellen können
- Damit wir sehen, wie sich Ihre Blutdruckwerte entwickeln, und
  auffällige Werte an Sie oder Ihre Angehörigen weitergeben können

**Wichtig:** Die Blutdruckdokumentation ist keine ärztliche Behandlung
und ersetzt keinen Arztbesuch.

&nbsp;

## Wo werden die Daten gespeichert?

Ihre Daten werden in einer geschützten Software (**entlast.de**) gespeichert.
Diese wird von Christoph Glunz betrieben. Die Server stehen in
**Deutschland** und sind verschlüsselt.

Nur die Mitarbeiter von [FIRMA], die für Ihre Betreuung zuständig sind,
können Ihre Daten sehen.

&nbsp;

## Wie lange werden die Daten gespeichert?

- **Rechnungen:** 10 Jahre (das ist gesetzlich vorgeschrieben)
- **Leistungsnachweise:** 5 Jahre
- **Blutdruckwerte:** bis zum Ende unserer Zusammenarbeit — auf Wunsch
  löschen wir sie jederzeit früher
- **Alle anderen Daten:** Werden gelöscht, wenn unsere Zusammenarbeit
  endet und keine Aufbewahrungspflicht mehr besteht

&nbsp;

## Sie können Ihre Einwilligung jederzeit zurücknehmen!

**Wichtig:** Sie können diese Einwilligung **jederzeit widerrufen** —
ohne Angabe von Gründen.

Das geht ganz einfach:
- Sagen Sie uns mündlich Bescheid, oder
- Schreiben Sie uns: [E-MAIL] / [ADRESSE], oder
- Rufen Sie uns an: [TELEFON]

Wenn Sie die Einwilligung widerrufen, löschen wir Ihre Gesundheitsdaten.
Was wir bis zum Widerruf gespeichert und genutzt haben, bleibt rechtmäßig.

Ein Widerruf kann allerdings dazu führen, dass wir die Abrechnung mit
Ihrer Pflegekasse nicht mehr durchführen können.

&nbsp;

---

&nbsp;

## Ihre Einwilligung

Ich habe die obigen Informationen gelesen und verstanden.

Ich willige **ausdrücklich** ein, dass [FIRMA] meine oben genannten
Gesundheitsdaten (insbesondere meinen Pflegegrad und — soweit erhoben —
meine Blutdruckwerte) zum Zweck der Leistungserbringung, der
Betreuungsdokumentation und der Abrechnung nach § 45b SGB XI verarbeitet
und in der Software entlast.de speichert.

Mir ist bekannt, dass ich diese Einwilligung **jederzeit und ohne
Angabe von Gründen widerrufen** kann.

&nbsp;

**Ort, Datum:** _______________________________________________

&nbsp;

**Unterschrift der betroffenen Person**
**(oder des/der gesetzlichen Betreuer/in):**

&nbsp;

_______________________________________________

Name in Druckbuchstaben: _______________________________________________

&nbsp;

*(Bei gesetzlicher Betreuung: Bitte Betreuungsvollmacht beifügen)*

---

&nbsp;

## Digitale Variante (Checkbox)

*Für die digitale Erfassung in entlast.de:*

- [ ] **Ich willige ein**, dass meine Gesundheitsdaten (insbesondere Pflegegrad und Versichertennummer) zum Zweck der Leistungserbringung und Abrechnung nach § 45b SGB XI in der Software entlast.de gespeichert und verarbeitet werden. Ich kann diese Einwilligung jederzeit formlos widerrufen.

Eingewilligt am: [DATUM]
Eingewilligt durch: [NAME DER PERSON / DES BETREUERS]
Erfasst durch: [MITARBEITER]

---

*Verantwortlich: [FIRMA], [ANSCHRIFT], [TELEFON], [E-MAIL]*
